...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImRect check_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(label_size.y + style.FramePadding.y * 1.1, label_size.y + style.FramePadding.y * 1.1));
    ItemSize(check_bb, style.FramePadding.y);

    ImRect total_bb = check_bb;
    if (label_size.x > 0)
        SameLine(0, style.ItemInnerSpacing.x);
    const ImRect text_bb(window->DC.CursorPos + ImVec2(1 , style.FramePadding.y -2), window->DC.CursorPos + ImVec2(5, style.FramePadding.y) + label_size);
    if (label_size.x > 0)
    {
    ...
    RenderFrame(check_bb.Min, check_bb.Max, (held && hovered) ? ImColor(17, 17, 17) : hovered ? ImColor( 17, 17, 17 ) : ImColor(17, 17, 17), true, style.FrameRounding);
    if (*v)
    {
        const float check_sz = ImMin(check_bb.GetWidth(), check_bb.GetHeight());
        const float pad = ImMax(0.0f, (float)(int)(check_sz / 7.0f));
...
bool ImGui::RadioButton(const char* label, bool active)
{
    ...
    if (active)
    {
        ...
        window->DrawList->AddCircleFilled(center, radius - pad, ImColor(32, 111, 234), 16);
    }
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImU32 frame_col = hovered ? ImColor(17, 17, 17) : ImColor(17, 17, 17);
    ...
    {
        ImU32 bg_col = (popup_open || hovered) ? ImColor(17, 17, 17) : ImColor(17, 17, 17);
        ImU32 text_col = ImColor(255, 255, 255);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (!temp_input_is_active)
    {
        const bool input_requested_by_tabbing = temp_input_allowed && (window->DC.LastItemStatusFlags & ImGuiItemStatusFlags_Focused) != 0;
        const bool clicked = (hovered && g.IO.MouseClicked[0]);
    ...
    // Draw frame
    const ImU32 frame_col = g.ActiveId == id ? ImColor(17, 17, 17) : hovered ? ImColor(17, 17, 17) : ImColor( 17, 17, 17 );
    RenderNavHighlight(frame_bb, id);
